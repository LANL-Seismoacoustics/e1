...
e1byts = (
    b'\x00H\x00<\x02\x00\x00\xc3\x93[\x8b\xed\xc0\x08L\xf9\xcc^\xc1\x04\xcf'
...
expected = np.array([
    309, 332, 336, 340, 377, 439, 494, 519, 539, 561,
    587, 611, 606, 586, 592, 636, 660, 654, 634, 602
], dtype=np.int32)
...
def test_compress_roundtrip():
    # compress the expected data
    compressed = e1.compress(expected)

    # Check block header:
    hdr = compressed[:4]
    block_len, nsamp = struct.unpack(">HH", hdr)
    print(f"Compressed block length = {block_len}")
    print(f"Compressed block sample count = {nsamp}")
    print(f"Compressed data size = {len(compressed)} bytes")

    # decompress the compressed result
    observed = e1.decompress(compressed, count)

    print("Original data:", expected)
    print("Decompressed data:", observed)

    np.testing.assert_array_equal(observed, expected)

    # Optional: check if byte-for-byte match with original bytes
    if compressed == e1byts:
        print("Compressed bytes exactly match e1byts.")
    else:
        print("Compressed bytes differ from original e1byts (this can be "
              "normal).")
