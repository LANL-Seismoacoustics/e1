...
def test_codec_matches_raw_e1_compression(memory_store):
    """Test that the codec stores exactly the same bytes as raw e1.compress() for a single chunk."""
    # Generate random data
    data = rand_int32(1000)
    
    # Compress directly with e1 library
    raw_compressed = e1.compress(data, datatype=b"e1")
    
    # Create a Zarr array with a single chunk (so the codec writes exactly one chunk)
    array = zarr.create(
        shape=data.shape,
        chunks=data.shape,  # Single chunk
        dtype='int32',
        store=memory_store,
        codecs=[E1Codec()]
    )
    
    # Write data via Zarr array (which uses the codec)
    array[:] = data
    
    # Retrieve the stored compressed bytes for the single chunk
    # The chunk key is 'c/0' for a single chunk array in the memory store
    chunk_key = 'c/0'
    stored_bytes = memory_store.get_sync(chunk_key).to_bytes()
    
    # The codec should store exactly the raw e1 compressed bytes (no extra header)
    assert stored_bytes == raw_compressed, \
        "Codec stored bytes do not match raw e1.compress() output"
...
def test_corrupted_data_handling(memory_store):
    ...
    with pytest.raises(Exception):  # Should raise some error from e1 decompression
        _ = array[:]
    
    # Test 2: Write corrupted garbage (no header)
    # Write 100 bytes of garbage
    memory_store.set_sync(chunk_key, buf_proto.buffer.from_bytes(b"\x00" * 100))
    
    with pytest.raises(Exception):  # Should raise decompression or checksum error
        _ = array[:]
    
    # Test 3: Write valid compressed data for 50 samples (no header)
    # This should fail because we are asking for 100 samples but only providing data for 50
    valid_50 = e1.compress(rand_int32(50), datatype=b"e1")
    memory_store.set_sync(chunk_key, buf_proto.buffer.from_bytes(valid_50))
    
    with pytest.raises(Exception):  # Should raise decompression error because sample count mismatch
        _ = array[:]
